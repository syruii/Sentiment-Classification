...
def generate_batch(data, batch_size, skip_window):
    ...
    # creates a list with length equal to span
    buffer = collections.deque(maxlen=span)
    if data_index + span > len(data):
        data_index = 0

    # append span for first word into buffer
    for _ in range(span):
        buffer.append(data[data_index])
        data_index = (data_index + 1) % len(data)
    # target will always be in the middle of the buffer
    target = skip_window
    for i in range(batch_size):
        # put all words except the middle into batch
        batch[i, : ] = [word for idx, word in enumerate(buffer) if idx != skip_window]
        labels[i, 0] = buffer[target]
        # shift buffer across the data
        buffer.append(data[data_index])
        data_index = (data_index + 1) % len(data)
